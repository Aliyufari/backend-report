...
export default function Modal({ show, onClose, action, loading, message }: Props) {
    ...
    return (
        <section onClick={handleClose} className={`modal w-full h-screen bg-[#333333AA] backdrop-blur-xs fixed top-0 left-0 flex justify-center items-center cursor-pointer transition-all duration-500 ${show ? "translate-y-0 opacity-100" : "-translate-y-[120vh] opacity-0"} p-6 z-50`}>
            <div className={`w-full max-w-[400px] bg-white rounded-md cursor-default transition-all duration-500 delay-500 ${show ? "opacity-100" : "opacity-0"} p-8`}>
                ...
                <div className="flex justify-center items-center gap-4 flex-wrap">
                    ...
                    <form action={action}>
                        <button disabled={loading} type="submit" className="rounded bg-primary-500 hover:bg-green-600 text-white transition-colors cursor-pointer flex justify-center items-center gap-1 py-2 px-4">
