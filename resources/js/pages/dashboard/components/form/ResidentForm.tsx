...
import { Camera, MoveLeft, CalendarIcon } from "lucide-react";
...
export default function ResidentForm() {
    ...
    return (
        <>
            <h2 className="font-semibold md:text-lg text-zinc-600 mb-1">Resident Application Form</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="relative w-full bg-white rounded-lg p-8 mb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
                    ...
                    <div>
                        <input type="hidden" id="dob" {...register("dob", {required: true})} required/>
                        ...
                        <Popover>
                            <PopoverTrigger asChild>
                                <Button
                                    variant={"outline"}
                                    className={cn(
                                        "w-full py-2 px-4 border rounded-md outline-none focus:border-primary transition-colors cursor-pointer hover:bg-transparent",
                                    )}
                                >
                                    {formValues.dob ? format(formValues.dob, "PPP") : <span className="font-light text-zinc-500">Pick a date</span> }
                            ...
                            <Calendar
                                mode="single"
                                selected={formValues.dob ? new Date(formValues.dob) : undefined}
                                onSelect={(date) => setValue("dob", date ? format(date, "yyyy-MM-dd") : "")}
                    ...
                    <div>
                        <label htmlFor="vin" className="block w-full mb-1">Voter's Identification Number (VIN):</label>
                        <input type="text" id="vin"
                            ...
                            required
                        />
                        {errors.vin ? <p className="text-secondary mt-1">VIN number is required</p> : null}
                    </div>
