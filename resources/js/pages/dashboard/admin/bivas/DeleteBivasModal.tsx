...
export default function DeleteBivasModal({ open, onClose, machine }: Props) {
    const { delete: destroy, processing } = useForm({});

    const handleDelete = () => {
        if (!machine) return;
        destroy(bivas.destroy(machine.id).url, {
            onSuccess: () => onClose(),
            onError:   () => {},
        });
    };
            ...
            >
                <div className="bg-card border border-border rounded-[14px] w-full max-w-md p-7
                    ...
                    <p className="font-['DM_Mono',monospace] text-[12px] text-muted-foreground leading-relaxed">
                        ...
                        <strong className="text-foreground">{machine.serial_number}</strong>?
