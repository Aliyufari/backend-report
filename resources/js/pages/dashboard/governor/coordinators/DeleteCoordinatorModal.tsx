...
import users from "@/routes/governor/users";
...
export default function DeleteCoordinatorModal({ open, onClose, user }: Props) {
    ...
    const handleDelete = () => {
        ...
        destroy(users.destroy(user.id).url, {
            onSuccess: () => onClose(),
            onError: () => {
                toast.error(errors.general ?? "Failed to delete coordinator. Please try again.");
            },
        });
    };
